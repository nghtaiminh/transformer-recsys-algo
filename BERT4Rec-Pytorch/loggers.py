...
class MetricGraphPrinter(AbstractBaseLogger):
    ...
    def log(self, *args, **kwargs):
        if self.key in kwargs:
            ...
            # Add for each epoch
            self.writer.add_scalar(self.group_name + '/' + self.graph_label + '_epoch', kwargs[self.key], kwargs['epoch'])
        else:
            self.writer.add_scalar(self.group_name + '/' + self.graph_label, 0, kwargs['accum_iter'])
            # Add for each epoch
            self.writer.add_scalar(self.group_name + '/' + self.graph_label + '_epoch', 0, kwargs['epoch'])
